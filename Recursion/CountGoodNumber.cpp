...
public:
    ...
    long long powr(long long x, long long n) {
        ...
        if(n & 1) {
            return ( x * powr(x,n-1) ) % modes;
        }
        return (powr((x*x)% modes,n/2)) % modes;
    }
    int countGoodNumbers(long long n) {
        //half of number
        long long numberOfOddPlaces = n/2;
        
        //half of number + remaining number
        long long numberOfEvenPlaces = n/2 + n%2;
        
        //formula
        return (powr(5, numberOfEvenPlaces) * powr(4, numberOfOddPlaces)) % modes;
    }
};
